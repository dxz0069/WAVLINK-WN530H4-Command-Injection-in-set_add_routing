# VulnDB Submission: 360 M5 cgitest.cgi get_installed_plugins Buffer Overflow via strcpy

## 1. Title

360 M5 cgitest.cgi get_installed_plugins Buffer Overflow via strcpy

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-BOF-001
...
- **Affected Component:** web/cgi-bin/cgitest.cgi ? get_installed_plugins @ 0x29acc
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

Stack-based buffer overflow in get_installed_plugins (0x29acc) of cgitest.cgi in 360 M5 firmware. User parameters (app_os, plugin_type, from, returnHideOpk, installed_plugin_type) read via get_form_value() flow to strcpy() without bounds checking. Dataflow confirmed: get_form_value → strcpy(stack_buffer). The 'IOS_AV' value reaches the strcpy sink.

## 5. Technical Details

**Affected Parameter(s):** app_os, plugin_type, from, returnHideOpk, installed_plugin_type, applet

- **Source**: get_form_value('app_os')
- **Sink**: strcpy() — no bounds checking
- **Dataflow**: get_form_value → strcpy confirmed (parameter: IOS_AV)
