# VulnDB Submission: 360 M5 cgitest.cgi mesh_put_update_file_cgi Buffer Overflow via strcpy

## 1. Title

360 M5 cgitest.cgi mesh_put_update_file_cgi Buffer Overflow via strcpy

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-BOF-004
...
- **Affected Component:** web/cgi-bin/cgitest.cgi ? mesh_put_update_file_cgi @ 0x58038
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

Stack-based buffer overflow in mesh_put_update_file_cgi (0x58038) of cgitest.cgi in 360 M5 firmware. User parameters (rom_md5, put_file, model, maclist) read via get_form_value() flow to strcpy() without bounds checking. Dataflow confirmed: get_form_value → strcpy(stack_buffer). The 'response' value reaches the strcpy sink.

## 5. Technical Details

**Affected Parameter(s):** rom_md5, put_file, model, maclist, applet

- **Source**: get_form_value('rom_md5')
...
applet=mesh_put_update_file_cgi&rom_md5=...[overflow_payload]...[shellcode]
```
