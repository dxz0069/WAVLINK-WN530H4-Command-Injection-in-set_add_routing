# VulnDB Submission: 360 M5 cgitest.cgi mesh_set_subnode_name_cgi Buffer Overflow via strcpy

## 1. Title

360 M5 cgitest.cgi mesh_set_subnode_name_cgi Buffer Overflow via strcpy

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-BOF-006
...
- **Affected Component:** web/cgi-bin/cgitest.cgi ? mesh_set_subnode_name_cgi @ 0x57b48
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

Stack-based buffer overflow in mesh_set_subnode_name_cgi (0x57b48) of cgitest.cgi in 360 M5 firmware. User parameters (mac, dev_name) read via get_form_value() flow to strcpy() without bounds checking. Dataflow confirmed: get_form_value → strcpy(stack_buffer). The 'response' value reaches the strcpy sink.

## 5. Technical Details

**Affected Parameter(s):** mac, dev_name, applet

- **Source**: get_form_value('mac')
...
applet=mesh_set_subnode_name_cgi&mac=...[overflow_payload]...[shellcode]
```
