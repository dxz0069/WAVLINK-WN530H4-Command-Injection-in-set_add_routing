# VulnDB Submission: 360 M5 cgitest.cgi mesh_set_update_cgi Buffer Overflow via strcpy

## 1. Title

360 M5 cgitest.cgi mesh_set_update_cgi Buffer Overflow via strcpy

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-BOF-007
...
- **Affected Component:** web/cgi-bin/cgitest.cgi ? mesh_set_update_cgi @ 0x55da0
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

Stack-based buffer overflow in mesh_set_update_cgi (0x55da0) of cgitest.cgi in 360 M5 firmware. User parameters (maclist) read via get_form_value() flow to strcpy() without bounds checking. Dataflow confirmed: get_form_value → strcpy(stack_buffer). The 'response' value reaches the strcpy sink.
...
applet=mesh_set_update_cgi&maclist=...[overflow_payload]...[shellcode]
```
