# VulnDB Submission: 360 360 P3 n360.cgi FUN_00406d98 OS Command Injection

## 1. Title

360 360 P3 n360.cgi FUN_00406d98 OS Command Injection

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-CMD-004
...
- **Product:** 360 360 P3 Router
- **Affected Version / Firmware:** 360P3-V2.0.11.59510.bin
- **Affected Component:** web/cgi-bin/n360.cgi ? FUN_00406d98 @ 0x406d98
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

OS command injection in FUN_00406d98 (0x406d98) of n360.cgi in 360 360 P3 firmware 360P3-V2.0.11.59510.bin. User parameters (type) read via get_form_value() flow to system() without sanitization. Pattern: Speed test — get_form_value→snprintf→system('echo ... > /tmp/test_speed')

## 5. Technical Details

**Affected Parameter(s):** type, applet

- **Source**: get_form_value('type')
- **Sink**: system()
- **Binary strings**: /tmp/test_speed, echo '"status":0,"band":0' > /tmp/test_speed
...
```http
POST /cgi-bin/n360.cgi HTTP/1.1
Host: <TARGET_IP>
Content-Type: application/x-www-form-urlencoded

applet=FUN_00406d98&type=1;id;
```
