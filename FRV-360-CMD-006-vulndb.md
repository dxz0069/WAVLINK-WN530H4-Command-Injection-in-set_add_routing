# VulnDB Submission: 360 360 M5 cgitest.cgi plugin_local_debug_set_cgi OS Command Injection

## 1. Title

360 360 M5 cgitest.cgi plugin_local_debug_set_cgi OS Command Injection

## 2. Vulnerability Summary

- **Internal Tracking ID:** FRV-360-CMD-006
...
- **Affected Component:** web/cgi-bin/cgitest.cgi ? plugin_local_debug_set_cgi @ 0x288ec
- **Architecture:** MIPS32 (Big Endian)

## 4. Description

OS command injection in plugin_local_debug_set_cgi (0x288ec) of cgitest.cgi in 360 360 M5 firmware M5-4.0.17.1566-rel-upgrade.bin. User parameters (localdebug) read via get_form_value() flow to system() without sanitization. Pattern: Debug mode — get_form_value→snprintf→system('touch /etc/config/plugins')

## 5. Technical Details

**Affected Parameter(s):** localdebug, applet

- **Source**: get_form_value('localdebug')
- **Sink**: system()
...
applet=plugin_local_debug_set_cgi&localdebug=1;id;
```
